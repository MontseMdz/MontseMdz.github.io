...
try{
    const response = await fetch('productos.json');// 1. Carga el archivo
    const productos = await response.json(); // 2. Analiza la respuesta como JSON
...
//Funciòn para mostrar los productos en el dom
    ...
    productos.forEach(product => {
        ...
        article.innerHTML = `
            ...
            <div class="card__cantidad" id="card__cantidad-${product.id}">
                ...
                <div class="card__numero" id="cantidad-${product.id}" data-id="${product.id}">
                    1
                </div>
...
function displayCart() {
    ...
    if(cart.length === 0){// ESTA PARTE ES PARA QUE SE MUESTRE EL LOGO DESPUES DE ELIMINAR UN PRODUCTO DEL CARRITO
        cartList.innerHTML = `
            <div class="items__img">
                <img src="imagenes/logo.png" alt="Logo carrito">
            </div>
            <p>Tu ropa aparecerán aquí</p>
        `;
    }else{
        cart.forEach(item => {
    ...
    cartHeader.textContent = `Tu carrito (${totalItems})`; //ME QUEDE EN EL MIN 52:10
...
//Funcion para cerrar el modal de pedido y resetear el estado del carrito
